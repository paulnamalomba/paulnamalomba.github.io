# Paul Namalomba Website Repository Management Script
...
# Create and cement release using gh
function Invoke-GitRelease {
    ...
    $existingRelease = gh release list | Select-String "^$VersionNumber\s"
    if ($existingRelease) {
        Write-Warning "Release $VersionNumber already exists in remote"
        return $false # Do not attempt to delete remote releases at all
    }
    
    # Create annotated release with message from file, but check first if the corresponding tag exists locally, if not create it first
    $existingTag = git tag -l $VersionNumber
    if (-not $existingTag) {
        Write-Info "Tag $VersionNumber does not exist locally, creating it first..."
        if (-not (Invoke-GitTag -VersionNumber $VersionNumber -Remote $Remote)) {
            Write-Error "Failed to create tag, cannot proceed with release"
            return $false
        }
    }
...
function Invoke-Release {
    ...
    Write-Host ""
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "  Paul Namalomba Website Release: $VersionNumber" -ForegroundColor Yellow
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    Write-Host "  https://github.com/paulnamalomba/paulnamalomba.github.io/releases/tag/$VersionNumber" -ForegroundColor Cyan
    Write-Host ""
}
...
switch ($Action.ToLower()) {
    ...
    "help" {
        Write-Host ""
        Write-Host "Paul Namalomba Website Repository Management Script" -ForegroundColor Cyan
        Write-Host "====================================" -ForegroundColor Cyan
        Write-Host ""
